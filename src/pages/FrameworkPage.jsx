...
export default function FrameworkPage(){
 const[activeKey,setActiveKey]=useState("remediation"),[activeStage,setActiveStage]=useState(0);const active=domains[activeKey];
 useEffect(()=>{const id=setInterval(()=>setActiveStage(s=>(s+1)%8),2500);return()=>clearInterval(id)},[activeKey]);
 return <>
 <section className="relative overflow-hidden px-5 pb-14 pt-28"><div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_38%,rgba(45,216,234,.12),transparent_28%),radial-gradient(circle_at_75%_42%,rgba(158,116,255,.09),transparent_43%)]"/><div className="relative mx-auto grid max-w-[1450px] items-center gap-12 lg:grid-cols-[1.02fr_.98fr]"><motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}}><div className="text-[11px] font-extrabold tracking-[.18em] text-cyan-300">PRODSECOPS FRAMEWORK · RISK-DRIVEN AI SECURITY OPERATIONS</div><h1 className="mt-5 font-[Manrope] text-6xl font-extrabold leading-[.94] tracking-[-.07em] md:text-8xl"><span className="text-sky-400">Prod</span>Sec<span className="text-emerald-400">Ops</span><br/>Framework</h1><h2 className="mt-6 max-w-4xl font-[Manrope] text-2xl font-bold leading-tight md:text-3xl">Orchestrate production security risk across remediation, detection, response, recovery, and compliance.</h2><p className="mt-6 max-w-3xl text-base leading-7 text-slate-300">ProdSecOps places 5-Dimensional Intelligence Risk Governance at the center. SecLabs, RGSM, and the 8-Stage Workflow provide the proving, control, tracking, and lifecycle capabilities used across five intelligence domains.</p><div className="mt-8 flex flex-wrap gap-3"><a href="#architecture" className="rounded-xl bg-gradient-to-r from-sky-500 to-cyan-400 px-5 py-3 text-sm font-extrabold">Explore the architecture</a><a href="#components" className="rounded-xl border border-sky-800 bg-[#06182b]/80 px-5 py-3 text-sm font-extrabold">Understand the components</a></div></motion.div><ArchitectureOrb activeKey={activeKey} setActiveKey={setActiveKey}/></div><div className="relative mx-auto mt-7 grid max-w-[1450px] gap-4 rounded-2xl border border-sky-900 bg-gradient-to-r from-[#06172a] via-[#082239] to-[#06172a] p-5 text-center md:grid-cols-3"><div><b className="block text-[11px] text-cyan-300">ONE CENTRAL ENGINE</b><span className="text-sm text-slate-300">5D Intelligence Risk Governance</span></div><div><b className="block text-[11px] text-cyan-300">THREE COMPONENTS</b><span className="text-sm text-slate-300">SecLabs · RGSM · 8-Stage Workflow</span></div><div><b className="block text-[11px] text-cyan-300">FIVE DOMAIN OPERATIONS</b><span className="text-sm text-slate-300">Remediation · SOC · IR · Resilience · Compliance</span></div></div></section>
